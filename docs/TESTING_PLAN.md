...
The synchronized Stage 7 title is **Agent Production Engineering: Harness, Loops & Graphs** (with localized equivalents); Multi-Agent, Orchestration, Handoff, Harness, Eval, Observability, Guardrail, Loop Engineering, and Graph Engineering remain visible and defined.
...
`scripts/test_agent_engineering_route.py` locks Stage 3 as the Agent Loop entry, Stage 4 as the framework／Workflow Graph entry, and Stage 7 as the Agent Production Engineering integration point for Harness, Loop, and Graph. It also locks the glossary boundary: Loop Engineering can happen in one long run or across sessions, Graph means an execution／workflow graph rather than GraphRAG, and both names have real industry／research usage without being misrepresented as one formal cross-vendor standard. The glossary must keep the course order separate from the five-layer control-scope order and must not fall back to the old Harness-only Stage 7 label.
